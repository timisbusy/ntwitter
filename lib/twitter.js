...
Twitter.prototype.userProfileImage = function(id, params, callback) {
  ...
  // using the v1 api for this call because it doesn't exist in 1.1  
  var v1Base = "https://api.twitter.com/1"

  var url = '/users/profile_image?' + querystring.stringify(params);
  // Do our own request, so we can return the 302 location header
  var request = this.oauth.get(v1Base + url,
    this.options.access_token_key,
    this.options.access_token_secret);
  request.on('response', function(response) {
