...
function Twitter(options) {
  ...
  this.options = extend({
    ...
    ads_base: 'https://ads-api.twitter.com/1',
    ads_sandbox_base: 'https://ads-api-sandbox.twitter.com/1',
    request_options: {
      headers: {
        Accept: '*/*',
        Connection: 'close',
...
Twitter.prototype.__buildEndpoint = function(path, base) {
  var bases = {
    ...
    'ads': this.options.ads_base,
    'ads_sandbox': this.options.ads_sandbox_base,
  };
  var endpoint = (bases.hasOwnProperty(base)) ? bases[base] : bases.rest;
  var skipJson = base === "ads" || base === "ads_sandbox";
...
Twitter.prototype.__request = function(method, path, params, callback) {
  ...
  // Pass url parameters if get, delete
  if (method === 'get') {
    options.qs = params;
  }
  ...
  this.request(options, function(error, response, data) {
    if (error) {
      ...
      if (typeof data.errors !== 'undefined') {
        callback(new Error(data.errors[0].message), data, response);
      }
...
Twitter.prototype.del = function(url, params, callback) {
